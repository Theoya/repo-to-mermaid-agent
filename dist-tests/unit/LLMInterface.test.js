...
const LLMInterface_1 = require("../../src/llm/LLMInterface");
// Mock implementation for testing
class MockLLMClient extends LLMInterface_1.LLMInterface {
    getProviderName() {
        return 'mock';
    }
    async processBucket(bucket, _previousSummary, _previousMermaid) {
        return {
    ...
    async generateSummary(files, _previousSummary) {
        return `Mock summary for ${files.length} files`;
    }
    async generateMermaid(_summary, _files, _previousMermaid) {
        return 'graph TD\nA[Test] --> B[Result]';
    }
...
describe('LLMInterface', () => {
    ...
    describe('processBucket', () => {
        ...
        it('should handle previous summary and mermaid', async () => {
            const response = await mockLLM.processBucket(mockBucket, 'Previous summary', 'Previous mermaid');
            expect(response.summary).toContain('Mock summary');
            expect(response.mermaid_content).toContain('graph TD');
        });
    });
    ...
    describe('generateMermaid', () => {
        ...
        it('should handle previous Mermaid', async () => {
            const mermaid = await mockLLM.generateMermaid('Test summary', mockFiles, 'Previous mermaid');
            expect(mermaid).toContain('graph TD');
        });
    });
    ...
    describe('createSystemPrompt', () => {
        it('should create system prompt', () => {
            const prompt = mockLLM.createSystemPrompt();
    ...
    describe('createUserPrompt', () => {
        it('should create user prompt with files', () => {
            const prompt = mockLLM.createUserPrompt(mockFiles);
            expect(prompt).toContain('test.js');
            expect(prompt).toContain('console.log');
            expect(prompt).toContain('JSON');
        });
        it('should include previous context when provided', () => {
            const prompt = mockLLM.createUserPrompt(mockFiles, 'Previous summary', 'Previous mermaid');
    ...
    describe('getLanguageFromExtension', () => {
        it('should return correct language for known extensions', () => {
            const getLanguage = mockLLM.getLanguageFromExtension;
    ...
    describe('parseResponse', () => {
        it('should parse JSON response', () => {
            ...
            const parsed = mockLLM.parseResponse(jsonResponse);
        ...
        it('should handle non-JSON response', () => {
            ...
            const parsed = mockLLM.parseResponse(textResponse);
    ...
    describe('cleanMermaidContent', () => {
        it('should clean Mermaid content', () => {
            ...
            const cleaned = mockLLM.cleanMermaidContent(dirtyContent);
            expect(cleaned).toBe('graph TD\nA --> B');
        });
        it('should add flowchart prefix if needed', () => {
            const content = 'A --> B';
            const cleaned = mockLLM.cleanMermaidContent(content);
