...
const FileDiscovery_1 = require("../../src/core/FileDiscovery");
...
describe('FileDiscovery', () => {
    ...
    beforeEach(() => {
        ...
        fileDiscovery = new FileDiscovery_1.FileDiscovery(config);
        jest.clearAllMocks();
    });
    describe('discoverFiles', () => {
        it('should load specific files when provided', async () => {
            ...
            mockedFs.stat.mockResolvedValueOnce({ size: 100 });
            mockedFs.readFile.mockResolvedValueOnce(mockContent1);
            mockedFs.stat.mockResolvedValueOnce({ size: 200 });
        ...
        it('should discover files recursively using glob', async () => {
            ...
            mockedFs.stat.mockResolvedValue({ size: 100 });
        ...
        it('should discover files in single directory when not recursive', async () => {
            ...
            mockedFs.readdir.mockResolvedValue(mockEntries);
            mockedFs.stat.mockResolvedValue({ size: 100 });
            mockedFs.readFile.mockResolvedValue('test content');
