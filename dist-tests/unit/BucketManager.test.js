...
const BucketManager_1 = require("../../src/core/BucketManager");
describe('BucketManager', () => {
    let bucketManager;
    let mockFiles;
    beforeEach(() => {
        bucketManager = new BucketManager_1.BucketManager(1000, 0.9); // 1000 tokens max, 90% threshold
        mockFiles = [
            {
                path: 'file1.js',
    ...
    describe('addFileToBucket', () => {
        ...
        it('should not add file if it exceeds capacity', () => {
            ...
            const newFile = mockFiles[1]; // 300 tokens
            const result = bucketManager.addFileToBucket(bucket, newFile);
            // With 400k hard limit, this should actually fit
            expect(result).toBe(true);
            expect(bucket.files).toHaveLength(2);
            expect(bucket.total_tokens).toBeLessThanOrEqual(1100);
        });
    });
