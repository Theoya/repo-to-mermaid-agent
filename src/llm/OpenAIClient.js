...
const LLMInterface_1 = require("./LLMInterface");
class OpenAIClient extends LLMInterface_1.LLMInterface {
    constructor(apiKey, model = 'gpt-4', maxTokens = 8000, temperature = 0.1) {
     ...
     */
    async processBucket(bucket, previousSummary, previousMermaid) {
        ...
        try {
            ...
            if (this.model === 'gpt-5') {
                ...
                const outputText = response.input?.find((item) => item.role === 'assistant' && item.content?.[0]?.type === 'output_text');
     ...
     */
    async generateSummary(files, previousSummary) {
...
Be concise but thorough in your analysis.`;
     ...
     */
    async generateMermaid(summary, files, previousMermaid) {
